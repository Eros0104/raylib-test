...
int main(void) {
  ...
  // is required)
  Texture2D texture =
      LoadTexture("assets/cat-sprites/catspritesx22.png"); // Texture loading

  Rectangle frameRec = {0.0f, 0.0f, 30.0f, 30.0f};
  float frameStartingPoint[] = {0, 42, 84, 126, 164};
  int frameWidth[] = {38, 38, 38, 34, 40};
  int currentFrame = 0;
  int framesCounter = 0;
  ...
  while (!WindowShouldClose()) {
    ...
    if (framesCounter >= (60 / framesSpeed)) {
      ...
      frameRec.width = frameWidth[currentFrame];
      frameRec.x = frameStartingPoint[currentFrame];
    ...
    DrawTextureRec(texture, frameRec, texPos, WHITE);
